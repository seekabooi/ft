...
def print_table(results: Dict[int, Dict], start_round: int):
    """打印对比表格"""
    if not results:
        print("❌ 没有找到任何轮次数据")
        return
    ...
    # 额外：计算退休趋势
    # 检查 DEPRECATED + ARCHIVE 是否在增加
    deprecated_trend = [int(row[3]) + int(row[4]) for row in data]  # DEPRECATED + ARCHIVE
    if len(deprecated_trend) >= 2:
        first = deprecated_trend[0]
        last = deprecated_trend[-1]
        change = last - first
        if change > 0:
            print(f"✅ 退休策略（DEPRECATED+ARCHIVE）从 {first} 增加到 {last}（+{change}），退休机制正在生效")
        elif change == 0:
            print(f"ℹ️ 退休策略（DEPRECATED+ARCHIVE）保持 {first}，没有变化")
        else:
            print(f"⚠️ 退休策略（DEPRECATED+ARCHIVE）从 {first} 减少到 {last}（{change}），可能是归档后删除，请检查")

    # 检查 ACTIVE 趋势
    active_trend = [int(row[2]) for row in data]
    if len(active_trend) >= 2:
...
def main():
    ...
    print_table(results, args.start_round)
